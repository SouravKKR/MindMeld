...
var nodeJsByteUtils = {
  ...
  encodeUTF8Into(buffer2, source, byteOffset) {
    const trueByteLength = Buffer.byteLength(source, "utf8");
    const latinBytesWritten = tryWriteBasicLatin(buffer2, source, byteOffset);
    if (latinBytesWritten != null) {
      return trueByteLength;
    }
    nodeJsByteUtils.toLocalBufferType(buffer2).write(source, byteOffset, void 0, "utf8");
    return trueByteLength;
  },
...
var webByteUtils = {
  ...
  encodeUTF8Into(uint8array, source, byteOffset) {
    ...
    const writeLength = Math.min(bytes.length, Math.max(0, uint8array.length - byteOffset));
    if (writeLength > 0) {
      uint8array.set(bytes.subarray(0, writeLength), byteOffset);
    }
...
function serialize(object, options = {}) {
  ...
  let serializationIndex = serializeInto(buffer, object, checkKeys, 0, 0, serializeFunctions, ignoreUndefined, null);
  if (serializationIndex > buffer.length) {
    buffer = ByteUtils.allocate(serializationIndex);
    serializationIndex = serializeInto(buffer, object, checkKeys, 0, 0, serializeFunctions, ignoreUndefined, null);
  }
