...
class PreferredModelTier
{
    ...
    static hydrate()
    {
        ...
        PreferredModelTier.#hydratePromise = (async () =>
        {
            try
            {
                const bExists = await Persistence.exists(LocalLlmDownloadConstants.LOCAL_PREFERRED_TIER_PERSISTENCE_KEY);
                if (!bExists)
                {
                    PreferredModelTier.#cachedTier = PreferredModelTier.getDefaultTier();
                    return;
                }
                const record = await Persistence.read(LocalLlmDownloadConstants.LOCAL_PREFERRED_TIER_PERSISTENCE_KEY, dataFormats.JSON);
    ...
    static async setCurrentTier(tier)
    {
        ...
        try
        {
            await Persistence.write(LocalLlmDownloadConstants.LOCAL_PREFERRED_TIER_PERSISTENCE_KEY, { tier, at: Date.now() }, dataFormats.JSON);
        }
        catch (writeError)
        {
        ...
        window.dispatchEvent(new CustomEvent(LocalLlmDownloadEvents.PREFERRED_TIER_CHANGED,
        {
            detail: { tier }
        }));
    }
