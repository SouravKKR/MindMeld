...
import LocalLlmPromptBuilder from "./LocalLlmPromptBuilder.js";
...
class LocalAskAiRunner
{
    ...
    static async openStream(requestPayload, abortSignal)
    {
        ...
        const builtPrompt = LocalLlmPromptBuilder.build(
        {
            modelKey: modelKey,
        ...
        return new ReadableStream(
        {
            async start(streamController)
            {
                ...
                const handleAbort = () =>
                {
                    LocalLlmSessionController.interrupt();
                };
                if (abortSignal)
                {
                ...
                try
                {
                    await LocalLlmSessionController.generate(builtPrompt, (deltaText) =>
                    {
    ...
    {
        return LocalLlmPromptBuilder.isPromptModeSupported(LocalAskAiRunner.#resolvePromptMode(promptMode));
    }
