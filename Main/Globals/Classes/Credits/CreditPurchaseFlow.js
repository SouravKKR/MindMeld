 ...
 * Razorpay handling: quote (Options) → pick quantity (CreditPurchaseDialog)
 * → Initiate (server-priced order) → Razorpay checkout → Verify → refresh
 * the cached user so the new balance shows everywhere immediately.
 *
 ...
 * is still completed server-side by the Razorpay webhook — the balance then
    ...
    static async #openPaymentCheckout(initiateResponse)
    {
        ...
        return await new Promise((resolve) =>
        {
            const options =
            {
                ...
                name: "MindMeld",
                description: `${quotedCredits} MindMeld credits`,
                handler: async (razorpayResponse) =>
                {
                    ...
                    if (verifyResponse.ok)
                    {
                        ...
                        // Pull the fresh user so the new balance shows in
                        // Settings, the profile component and every other
                        // consumer of window["user"] immediately.
