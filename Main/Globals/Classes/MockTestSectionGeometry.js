import { sectionQuestionCountModes } from "../Enumerations/SectionQuestionCountModes.js";
import { sectionMarksModes } from "../Enumerations/SectionMarksModes.js";
 ...
 * question, and the section's total marks — but the user only ever enters two
 * of them. Which two depends on the marks mode:
 *
 ...
 *                         entered, and the question count is derived as the
 *                         band of counts that can add up to that budget:
 *                         ceil(total / maximum) .. floor(total / minimum).
 *
 * That is what makes "each question is worth 4-10 marks and some number of them
 * sum to 20" expressible: the answer is 2..5 questions, and nothing else has to
 * be pinned down.
 *
 ...
 * back-derived from those two, so a stored template or an already-queued task
 * keeps meaning exactly what it meant when it was written.
 *
 * This class is mirrored by hand into Dock/Globals/Classes/Generation and
 * Agent/Globals/Classes/Generation. The three copies MUST agree — the client
 * blocks on them, the server re-checks them, and assembly realises them.
 */
class MockTestSectionGeometry
{
    ...
    {
        if (sectionEntry?.marksMode === sectionMarksModes.RANGE_PER_QUESTION)
        {
    ...
    /**
     * True when the question count is not the user's to type — it falls out of
     * the marks budget instead.
     */
    ...
    /**
     * Marks per question for a UNIFORM section. Falls back to deriving it from
     * a legacy entry's totalMarks / questionCount pair, then to the paper's
     * default correct-marks value, so a section written before this field
     * existed still reports the marks it was always worth.
     */
    static resolveMarksPerQuestion(sectionEntry, fallbackMarksPerQuestion)
    {
        const configuredMarks = sectionEntry?.marksPerQuestion;
        if (typeof configuredMarks === "number" && Number.isFinite(configuredMarks) && configuredMarks > 0)
        {
            return configuredMarks;
        }

        const legacyTotalMarks = sectionEntry?.totalMarks;
        const legacyQuestionCount = sectionEntry?.questionCount;
    ...
    /**
     * The per-question marks band of a RANGE_PER_QUESTION section. A missing or
     * inverted maximum collapses to the minimum rather than producing an empty
     * band, so callers never have to defend against min > max.
     */
    static resolveMarksPerQuestionBand(sectionEntry)
    {
        const rawMinimum = sectionEntry?.marksPerQuestionMin;
        const rawMaximum = sectionEntry?.marksPerQuestionMax;
    ...
    {
        const configuredTotal = sectionEntry?.totalMarks;
        return (typeof configuredTotal === "number" && Number.isFinite(configuredTotal) && configuredTotal > 0) ? configuredTotal : 0;
    }

    /**
     * How many questions this section can hold, as an inclusive band.
     *
     * For a marks-driven section the band is derived from the budget; for a
     * count-driven one it is what the user typed. An unachievable configuration
     * returns a zero band — callers should ask describeValidationFailure why
    ...
    static resolveQuestionCountBand(sectionEntry)
    {
        ...
        const rawCount = sectionEntry?.questionCount;
    ...
    /**
     * The marks this section can be worth, as an inclusive band. Marks-driven
     * sections are a single value by construction — the budget IS the total.
     */
    ...
    /**
     * The count this section is most likely to realise. Count-range sections
     * honour their sampling weights (a template can say "3 questions much more
     * often than 6"); everything else is the midpoint of its band.
     */
    ...
    /**
     * The first thing wrong with this section, phrased for the user, or null
     * when it is configured coherently.
     *
     * Both the editor and the server call this, so a rejection reads the same
    ...
    /**
     * Checks the whole list, then reconciles it against the paper's own
     * question count.
     *
     * The reconciliation only applies when the user pinned that count manually.
     * On AUTOMATIC the sections are what decide the paper's size, so there are
     * not two numbers to disagree.
     */
...
export default MockTestSectionGeometry;
