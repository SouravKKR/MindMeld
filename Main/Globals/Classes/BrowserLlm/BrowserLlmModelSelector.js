import BrowserLlmModelCatalogue from "../../Constants/BrowserLlmModelCatalogue.js";
import BrowserLlmSelectionOutcome from "./BrowserLlmSelectionOutcome.js";
import { browserLlmExecutionBackends } from "../../Enumerations/BrowserLlmExecutionBackends.js";
import { browserLlmUnavailableReasons } from "../../Enumerations/BrowserLlmUnavailableReasons.js";


/**
 * BrowserLlmModelSelector
 *
 ...
 * Common/Constants/BrowserLlmModelCatalogue.json — a model's backend, its
 ...
 * described by a BrowserLlmDeviceProfile the caller supplies, and the
 ...
 */
class BrowserLlmModelSelector
{
    /**
     * @param {BrowserLlmDeviceProfile} deviceProfile     what this device can do
     ...
     *
     * @returns {BrowserLlmSelectionOutcome}
     */
    static select(deviceProfile, availableModelKeys, forcedModelKey = null)
    {
        ...
        {
            return new BrowserLlmSelectionOutcome({ unavailableReason: browserLlmUnavailableReasons.HANDHELD_DEVICE });
        }

        const orderedKeys = Array.isArray(BrowserLlmModelCatalogue.ORDER) ? BrowserLlmModelCatalogue.ORDER : [];
        const availableKeySet = new Set(Array.isArray(availableModelKeys) ? availableModelKeys : []);

        if (availableKeySet.size === 0)
        {
            return new BrowserLlmSelectionOutcome({ unavailableReason: browserLlmUnavailableReasons.NO_MODEL_PROVISIONED });
        }
        ...
        if (forcedModelKey && availableKeySet.has(forcedModelKey) && BrowserLlmModelCatalogue[forcedModelKey])
        {
            return new BrowserLlmSelectionOutcome({ modelKey: forcedModelKey, preferredModelKey: forcedModelKey });
        }
        ...
        for (const modelKey of orderedKeys)
        {
            ...
            if (!BrowserLlmModelSelector.#isBackendSupported(descriptor, deviceProfile))
            {
                continue;
            }
            bAnyBackendSupported = true;

            if (!BrowserLlmModelSelector.#areGpuRequirementsMet(descriptor, deviceProfile))
            {
                bAnyBlockedByGpuLimits = true;
                continue;
            }

            if (!BrowserLlmModelSelector.#isDeviceMemorySufficient(descriptor, deviceProfile))
            {
        ...
        {
            return new BrowserLlmSelectionOutcome(
            {
                unavailableReason: BrowserLlmModelSelector.#resolveUnavailableReason(
        ...
        candidateKeys.sort((firstKey, secondKey) =>
            BrowserLlmModelSelector.#rankOf(firstKey) - BrowserLlmModelSelector.#rankOf(secondKey));

        const chosenModelKey = candidateKeys[0];
        const preferredModelKey = BrowserLlmModelSelector.#findPreferredModelKey(orderedKeys, availableKeySet);

        return new BrowserLlmSelectionOutcome(
        {
            modelKey: chosenModelKey,
            preferredModelKey: preferredModelKey,
            degradeNote: BrowserLlmModelCatalogue[chosenModelKey].displayNote || null
        });
    }
    ...
    static #findPreferredModelKey(orderedKeys, availableKeySet)
    {
        ...
        for (const modelKey of orderedKeys)
        {
            if (!availableKeySet.has(modelKey) || !BrowserLlmModelCatalogue[modelKey])
            {
                continue;
            }
            const rank = BrowserLlmModelSelector.#rankOf(modelKey);
    ...
    {
        const backendValue = browserLlmExecutionBackends[descriptor.executionBackend];

        if (backendValue === browserLlmExecutionBackends.WEBGPU)
        {
            return deviceProfile.isWebGpuAvailable();
        }
        if (backendValue === browserLlmExecutionBackends.WASM)
        {
    ...
    {
        if (browserLlmExecutionBackends[descriptor.executionBackend] !== browserLlmExecutionBackends.WEBGPU)
        {
            return true;
        }
    ...
    static #resolveUnavailableReason(bAnyBackendSupported, bAnyBlockedByGpuLimits, bAnyBlockedByDeviceMemory)
    {
        ...
        {
            return browserLlmUnavailableReasons.NO_SUPPORTED_BACKEND;
        }
        if (bAnyBlockedByGpuLimits)
        {
            return browserLlmUnavailableReasons.GPU_LIMITS_TOO_LOW;
        }
        if (bAnyBlockedByDeviceMemory)
        {
            return browserLlmUnavailableReasons.DEVICE_MEMORY_TOO_LOW;
        }
        return browserLlmUnavailableReasons.NO_MODEL_PROVISIONED;
    }
    ...
    {
        return modelKey && BrowserLlmModelCatalogue[modelKey] ? BrowserLlmModelCatalogue[modelKey] : null;
    }
}

export default BrowserLlmModelSelector;
