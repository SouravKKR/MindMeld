import BrowserLlmDeviceProfile from "./BrowserLlmDeviceProfile.js";
...
/**
 * BrowserLlmDeviceProbe
 *
 * Asks the platform, once, what this device can do, and hands back a
 * BrowserLlmDeviceProfile. It reports facts and decides nothing — every
 * judgement about what those facts mean for a given model lives in
 * BrowserLlmModelSelector, where it can be tested without a browser.
 *
...
class BrowserLlmDeviceProbe
{
    ...
    static probe()
    {
        if (BrowserLlmDeviceProbe.#probePromise)
        {
            return BrowserLlmDeviceProbe.#probePromise;
        }

        BrowserLlmDeviceProbe.#probePromise = (async () =>
        {
            const profile = await BrowserLlmDeviceProbe.#buildProfile();
            BrowserLlmDeviceProbe.#cachedProfile = profile;
            return profile;
        })();
    ...
    {
        return BrowserLlmDeviceProbe.#cachedProfile;
    }
    ...
    static getForcedModelKey()
    {
        ...
        try
        {
            ...
            const forcedModelKey = searchParameters.get(BrowserLlmDownloadConstants.MODEL_OVERRIDE_QUERY_PARAMETER);
    ...
    {
        BrowserLlmDeviceProbe.#cachedProfile = null;
        BrowserLlmDeviceProbe.#probePromise = null;
        try
        {
        ...
        catch (writeError)
        {
            ...
            console.warn(`[BrowserLlmDeviceProbe] Could not persist the graphics-unusable flag: ${writeError?.message || writeError}`);
        }
    }
    ...
    static async #readGraphicsUnusableFlag()
    {
        try
        {
            const bExists = await Persistence.exists(BrowserLlmDownloadConstants.LOCAL_GRAPHICS_UNUSABLE_PERSISTENCE_KEY);
            if (!bExists)
            {
                return false;
            }
        ...
        {
            console.warn(`[BrowserLlmDeviceProbe] Could not read the graphics-unusable flag: ${readError?.message || readError}`);
            return false;
        }
    }
    ...
    static #detectHandheldDevice()
    {
        ...
        if (BrowserLlmDeviceProbe.HANDHELD_USER_AGENT_PATTERN.test(userAgentText))
        {
            return true;
        }
    ...
    static async #buildProfile()
    {
        ...
        const hardwareConcurrency = typeof navigator !== "undefined" && typeof navigator.hardwareConcurrency === "number"
            ...
            : 1;
        const bHandheldDevice = BrowserLlmDeviceProbe.#detectHandheldDevice();
        ...
        // never the problem.
        const bGraphicsProvenUnusable = await BrowserLlmDeviceProbe.#readGraphicsUnusableFlag();

        if (typeof navigator === "undefined" || !navigator.gpu || bGraphicsProvenUnusable)
        {
            if (bGraphicsProvenUnusable)
            {
                console.warn("[BrowserLlmDeviceProbe] Graphics previously lost its device on this machine — reporting WebGPU as unavailable so the processor backend is selected.");
            }
            return new BrowserLlmDeviceProfile(
            {
        ...
        try
        {
            ...
            {
                console.warn("[BrowserLlmDeviceProbe] WebGPU is a software fallback adapter — reporting graphics as unavailable so the processor backend is chosen directly.");
                return new BrowserLlmDeviceProfile(
                {
                    bWebGpuAvailable: false,
                    bWebAssemblyAvailable: bWebAssemblyAvailable,
            ...
            return new BrowserLlmDeviceProfile(
            {
                ...
                bShaderF16Supported: Boolean(adapterFeatures && typeof adapterFeatures.has === "function"
                    && adapterFeatures.has(BrowserLlmDeviceProbe.SHADER_F16_FEATURE_NAME)),
        ...
        catch (probeError)
        {
            ...
            console.warn(`[BrowserLlmDeviceProbe] WebGPU adapter request failed: ${probeError?.message || probeError}`);
            return new BrowserLlmDeviceProfile(
            {
                bWebGpuAvailable: false,
                bWebAssemblyAvailable: bWebAssemblyAvailable,
...
export default BrowserLlmDeviceProbe;
