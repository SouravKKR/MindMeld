...
class BrowserLlmPromptBuilder
{
    ...
    {
        return BrowserLlmPromptBuilder.SUPPORTED_PROMPT_MODE_NAMES
    ...
    {
        return BrowserLlmPromptBuilder.getSupportedPromptModes().includes(promptMode);
    }
    ...
    {
        const descriptor = BrowserLlmModelSelector.getDescriptor(modelKey);
        const contextWindowTokens = descriptor && Number.isFinite(descriptor.contextWindowTokens)
            ? descriptor.contextWindowTokens
            : 2048;
        const usableTokens = Math.max(256, contextWindowTokens - BrowserLlmPromptBuilder.RESERVED_OUTPUT_TOKENS);
        return Math.floor(usableTokens * BrowserLlmPromptBuilder.APPROXIMATE_CHARACTERS_PER_TOKEN);
    }
    ...
    {
        const promptBudget = BrowserLlmPromptBuilder.getPromptBudgetCharacters(modelKey);
        const deckBudget = Math.floor(promptBudget * BrowserLlmPromptBuilder.DECK_CONTEXT_BUDGET_SHARE);
    ...
    {
        const promptBudgetCharacters = BrowserLlmPromptBuilder.getPromptBudgetCharacters(modelKey);
        const promptModeName = BrowserLlmPromptBuilder.#PROMPT_MODE_NAME_LOOKUP.get(promptMode) || "EXPLAIN";
        const contextKindName = BrowserLlmPromptBuilder.#CONTEXT_KIND_NAME_LOOKUP.get(contextKind) || "CARD";

        const systemPrompt = BrowserLlmPromptBuilder.#truncate(
            BrowserLlmPromptBuilder.SYSTEM_PROMPT,
            Math.floor(promptBudgetCharacters * BrowserLlmPromptBuilder.SYSTEM_PROMPT_BUDGET_SHARE)
        );

        const promptSections = [];
        promptSections.push(BrowserLlmPromptBuilder.#buildContextSection(
            contextKindName,
            contextPayload,
        ...
        {
            const selectionBudget = Math.floor(promptBudgetCharacters * BrowserLlmPromptBuilder.SELECTED_TEXT_BUDGET_SHARE);
            promptSections.push(`Highlighted fragment:\n${BrowserLlmPromptBuilder.#truncate(trimmedSelection, selectionBudget)}`);
        }
        ...
        if (trimmedQuery.length > 0)
        {
            const queryBudget = Math.max(
                BrowserLlmPromptBuilder.MINIMUM_USER_QUERY_CHARACTERS,
                Math.floor(promptBudgetCharacters * BrowserLlmPromptBuilder.USER_QUERY_BUDGET_SHARE)
            );
            promptSections.push(`The learner asks:\n${BrowserLlmPromptBuilder.#truncate(trimmedQuery, queryBudget)}`);
        }

        const instruction = BrowserLlmPromptBuilder.INSTRUCTIONS[promptModeName] || BrowserLlmPromptBuilder.INSTRUCTIONS.EXPLAIN;
        promptSections.push(`${instruction}\n\n${BrowserLlmPromptBuilder.STYLE_CLAUSE}`);

        const userPrompt = BrowserLlmPromptBuilder.#fitToBudget(
            promptSections,
        ...
        return {
            ...
            maximumNewTokens: BrowserLlmPromptBuilder.MAXIMUM_NEW_TOKENS[promptModeName]
                || BrowserLlmPromptBuilder.MAXIMUM_NEW_TOKENS.EXPLAIN,
        };
    }
    ...
    static #buildContextSection(contextKindName, contextPayload, promptBudgetCharacters)
    {
        ...
        {
            const materialBudget = Math.floor(promptBudgetCharacters * BrowserLlmPromptBuilder.STUDY_MATERIAL_BUDGET_SHARE);
            const materialText = BrowserLlmPromptBuilder.#toPlainText(payload.content);
            return `Study material:\n${BrowserLlmPromptBuilder.#truncate(materialText, materialBudget)}`;
        }

        if (contextKindName === "DECK")
        {
            return BrowserLlmPromptBuilder.#buildDeckContextSection(payload, promptBudgetCharacters);
        }

        const fieldBudget = Math.floor(promptBudgetCharacters * BrowserLlmPromptBuilder.CARD_FIELD_BUDGET_SHARE);
        const questionText = BrowserLlmPromptBuilder.#truncate(BrowserLlmPromptBuilder.#toPlainText(payload.question), fieldBudget);
        const answerText = BrowserLlmPromptBuilder.#truncate(BrowserLlmPromptBuilder.#toPlainText(payload.answer), fieldBudget);
    ...
    {
        const deckBudget = Math.floor(promptBudgetCharacters * BrowserLlmPromptBuilder.DECK_CONTEXT_BUDGET_SHARE);
        ...
        for (const snippet of snippets)
        {
            const renderedSnippet = snippet && snippet.kind === "STUDY_MATERIAL"
                ? `From a study material:\n${BrowserLlmPromptBuilder.#toPlainText(snippet.content)}`
                : `From a flashcard:\nQ: ${BrowserLlmPromptBuilder.#toPlainText(snippet && snippet.question)}\nA: ${BrowserLlmPromptBuilder.#toPlainText(snippet && snippet.answer)}`;
        ...
        const renderedConversation = conversation
            .map((turn) => `${turn && turn.role === "assistant" ? "You" : "Learner"}: ${BrowserLlmPromptBuilder.#toPlainText(turn && turn.text)}`)
            .join("\n");
        ...
        {
            sections.push(`Earlier in this conversation:\n${BrowserLlmPromptBuilder.#truncate(renderedConversation, Math.floor(deckBudget * 0.3))}`);
        }
    ...
    static #fitToBudget(promptSections, budgetCharacters)
    {
        ...
        return BrowserLlmPromptBuilder.#truncate(sections.join("\n\n"), usableBudget);
    }
...
export default BrowserLlmPromptBuilder;
