class SolvingStepsDirective:
    ...
    SHOW_SOLVING_STEPS_BLOCK = (
        ...
        "from the question statement to the expected answer. Do not just restate the expected answer. "
        "FORMAT: emit `solvingSteps` as HTML. Wrap each step in <p>...</p>. Use <strong>...</strong> or "
        "<em>...</em> for emphasis. Use <ol><li>...</li></ol> if numbered ordering is clearer than separate "
        "paragraphs. Inline math goes inside \\(...\\) and display math inside \\[...\\] — these are KaTeX "
        "delimiters; do not use $...$ or $$...$$. Do not use Markdown (no `**`, no `_`, no `# `). "
        "For purely subjective / opinion / recall-only questions where there is no procedural path "
