...
class GoogleVertexAiProvider(AutomationProvider):
    """
    Enterprise Google Vertex AI provider.

    This is a behavioural twin of GeminiProvider — same request shaping,
    retry policy, streaming, image generation and credit metering — but it
    talks to the **Vertex AI** backend of the unified google-genai SDK
    instead of the Gemini Developer API. It is a drop-in replacement: every
    model tuple in ModelPool reads (model_string, ProviderClass), so the
    transition to Vertex is a one-line swap of GeminiProvider for
    GoogleVertexAiProvider per model (or a project-wide find/replace) with
    no change to any calling workflow.

    The provider is intentionally kept ready but NOT wired into ModelPool;
    nothing routes to it until those tuples are switched.

    How it differs from GeminiProvider (these are the ONLY semantic
    differences — everything else is mirrored verbatim):
      ...
      2. Video inputs. The Files API (client.files.upload) is a
         ...
         Vertex accepts media either inline (bytes) or as a gs:// GCS URI.
         We therefore read the local video into bytes and attach it inline
         instead of uploading it. See __build_video_part.
    """
    ...
    #  - 5xx (UNAVAILABLE / INTERNAL / etc.): Vertex-side capacity blip.
    ...
    def __build_ipv4_httpx_clients() -> tuple[httpx.Client, httpx.AsyncClient]:
        timeout_config = httpx.Timeout(GoogleVertexAiProvider.__HTTPX_REQUEST_TIMEOUT_SECONDS)
    ...
    def __init__(self):
        self.__client = GoogleVertexAiProvider.__build_client()
    ...
    async def __fetch_url_content(self, url: str) -> str:
        try:
            ...
            truncated = text[:GoogleVertexAiProvider.GROUNDING_CONTENT_CHAR_BUDGET]
    ...
    async def __build_video_part(video_path: str, mime_type: str) -> types.Part:
        """
        Vertex has no Files API, so we attach the video inline. The file is
        read off the worker thread to avoid blocking the event loop. Inline
        media is subject to Vertex's per-request payload ceiling; oversized
        clips should be staged in GCS and referenced via a gs:// URI by the
        calling workflow instead.
        """
        video_bytes = await asyncio.to_thread(GoogleVertexAiProvider.__read_file_bytes, video_path)
    ...
    async def execute(self, request: AutomationRequest) -> AutomationResponse:
            ...
            match ctype:
                ...
                case AutomationContentTypes.VIDEO:
                    if isinstance(data, str):
                        ...
                        user_parts.append(await GoogleVertexAiProvider.__build_video_part(data, mime_type))
        ...
        usage_metadata = GoogleVertexAiProvider.__record_token_usage(
            response,
            request.get_model(),
            request.get_text_content(),
            GoogleVertexAiProvider.__safe_response_text(response),
        )
    ...
    async def stream_text(
        ...
        *,
        ...
        """
        Async generator that streams a Vertex AI text response chunk-by-chunk.
        ...
        while True:
            ...
            try:
                async with RedisSemaphore.slot(
                    bucket = model,
                    max_concurrent = GoogleVertexAiProvider.__resolve_concurrent_limit(model),
                    ...
                    citation_sources = GoogleVertexAiProvider.__extract_citation_sources(last_seen_chunk)
                    if citation_sources:
                ...
                if not GoogleVertexAiProvider.__is_transient_error(api_error):
                ...
                error_label = GoogleVertexAiProvider.__describe_transient_error(api_error)
                if attempt_index >= GoogleVertexAiProvider.MAX_TRANSIENT_RETRIES:
                    print(
                        f"[GoogleVertexAiProvider] stream_text {error_label} after "
                        f"{attempt_index} retries on model {model} — giving up."
                    )
                    yield { "type": "error", "message": str(api_error) }
                    return

                sleep_seconds = GoogleVertexAiProvider.__resolve_retry_delay_seconds(api_error, attempt_index)
                print(
                    f"[GoogleVertexAiProvider] stream_text {error_label} on model {model} "
    ...
    async def __generate_content_with_retry(self, model: str, contents, config):
        """
        Calls Vertex generate_content with two layers of protection against
        ...
        2. Inside the slot we run the actual call. If the API still
           ...
           estimate exceeded) or 5xx (Vertex-side capacity blip) — we
        ...
        while True:
            ...
            async with RedisSemaphore.slot(
                ...
                try:
                    ...
                    if attempt_index >= GoogleVertexAiProvider.MAX_TRANSIENT_RETRIES:
                        print(
                            f"[GoogleVertexAiProvider] {error_label} after "
                            f"{attempt_index} retries on model {model} — giving up."
                        )
                        raise
                    ...
                    sleep_seconds = GoogleVertexAiProvider.__resolve_retry_delay_seconds(api_error, attempt_index)
                    print(
                        f"[GoogleVertexAiProvider] {error_label} on model {model} "
                        f"(attempt {attempt_index + 1}/{GoogleVertexAiProvider.MAX_TRANSIENT_RETRIES}). "
                        f"Sleeping {sleep_seconds:.1f}s then retrying."
                    )
    ...
    # Vertex-side capacity / internal errors. Both retry on the same
    ...
    def __is_transient_error(api_error) -> bool:
        ...
        if status_code in GoogleVertexAiProvider._TRANSIENT_STATUS_CODES:
            return True

        stringified_error = str(api_error)
        for transient_keyword in GoogleVertexAiProvider._TRANSIENT_STATUS_KEYWORDS:
    ...
    def __resolve_retry_delay_seconds(client_error, attempt_index: int) -> float:
        seconds = GoogleVertexAiProvider.__extract_retry_delay_from_error(client_error)
        if seconds is None:
            ...
            seconds = GoogleVertexAiProvider.DEFAULT_RETRY_SLEEP_SECONDS * (2 ** attempt_index)
        return min(max(seconds, 1.0), GoogleVertexAiProvider.MAX_RETRY_SLEEP_SECONDS)
            ...
            if isinstance(error_payload, dict):
                ...
                for detail_entry in error_block.get("details", []) or []:
                    if detail_entry.get("@type", "").endswith("RetryInfo"):
                        ...
                        parsed = GoogleVertexAiProvider.__parse_duration_string(delay_string)
        ...
        while True:
            ...
            async with RedisSemaphore.slot(
                    ...
                    for chunk in self.__client.models.generate_content_stream(
                        ...
                        for part in chunk.parts:
                            if part.inline_data and part.inline_data.data:
                                ...
                                print(f"[GoogleVertexAiProvider] stream text: {part.text}")
                ...
                try:
                    ...
                    sleep_seconds = GoogleVertexAiProvider.__resolve_retry_delay_seconds(api_error, attempt_index)
                    print(
                        f"[GoogleVertexAiProvider] {error_label} on image stream (model {model}, "
                        f"attempt {attempt_index + 1}/{GoogleVertexAiProvider.MAX_TRANSIENT_RETRIES}). "
                        f"Sleeping {sleep_seconds:.1f}s then retrying."
                    )
    ...
    async def __fetch_image_generation(
        ...
        print(f"[GoogleVertexAiProvider] Starting image generation stream (model={request.get_model()})...")
        ...
        # Vertex-side capacity blip mid-batch instead of failing the task.
        ...
        if not image_buffers:
            print("[GoogleVertexAiProvider] Image generation stream produced no image data")
        ...
        print(f"[GoogleVertexAiProvider] Image generation complete — {len(outputs)} image(s), "
