...
const BrowserLlmModelCatalogue = require("../../Constants/BrowserLlmModelCatalogue");
const BrowserLlmDownloadConstants = require("../../Constants/BrowserLlmDownloadConstants");


/**
 * BrowserLlmModelRegistry
 *
 ...
 * (Common/Constants/BrowserLlmModelCatalogue.json), never code. Provisioning
...
class BrowserLlmModelRegistry
{
    ...
    static getEnabledModelKeys()
    {
        ...
        const configuredValue = String(process.env[BrowserLlmModelRegistry.ENABLED_MODELS_ENVIRONMENT_VARIABLE] || "").trim();
    ...
    static collectFiles(rootDirectory)
    {
        ...
        const walk = (currentDirectory, relativePrefix) =>
        {
            ...
            {
                console.error(`[BrowserLlmModelRegistry] Could not read ${currentDirectory}: ${readError.message}`);
                return;
            }
            ...
            for (const directoryEntry of directoryEntries)
            {
                ...
                if (BrowserLlmModelRegistry.EXCLUDED_FILE_NAME_PATTERN.test(entryName))
                {
                    continue;
                }
    ...
    static isModelComplete(descriptor, collectedFiles, modelDirectory)
    {
        ...
        const shardPaths = BrowserLlmModelRegistry.readShardPaths(descriptor, modelDirectory);
    ...
    static readShardPaths(descriptor, modelDirectory)
    {
        ...
        {
            console.error(`[BrowserLlmModelRegistry] Malformed ${descriptor.shardManifestFileName} in ${modelDirectory}: ${parseError.message}`);
            return [];
        }
    }
    ...
    {
        const modelsDirectory = path.join(assetsDirectory, BrowserLlmModelRegistry.MODELS_DIRECTORY_NAME);
        const signature = BrowserLlmModelRegistry.buildDirectorySignature(modelsDirectory);

        if (BrowserLlmModelRegistry.#cachedDescription !== null && BrowserLlmModelRegistry.#cachedSignature === signature)
        {
            return BrowserLlmModelRegistry.#cachedDescription;
        }

        const describedModels = [];

        for (const modelKey of BrowserLlmModelRegistry.getEnabledModelKeys())
        {
            const descriptor = BrowserLlmModelCatalogue[modelKey];
            if (!descriptor)
            {
                console.error(`[BrowserLlmModelRegistry] "${modelKey}" is in ORDER but absent from the catalogue body.`);
                continue;
            }
            ...
            const collectedFiles = BrowserLlmModelRegistry.collectFiles(modelDirectory);
            if (collectedFiles.length === 0)
            {
                continue;
            }

            const baseUrl = `${BrowserLlmDownloadConstants.ASSETS_BASE_PATH}/${descriptor.folderName}/`;
            const servableFiles = [];
            ...
            describedModels.push(
            {
                ...
                bComplete: BrowserLlmModelRegistry.isModelComplete(descriptor, collectedFiles, modelDirectory),
            });
        }

        BrowserLlmModelRegistry.#cachedDescription = describedModels;
        BrowserLlmModelRegistry.#cachedSignature = signature;
    ...
    static resetCache()
    {
        BrowserLlmModelRegistry.#cachedDescription = null;
        BrowserLlmModelRegistry.#cachedSignature = null;
    }
}

module.exports = BrowserLlmModelRegistry;
