/**
 * VerifyLocalLlmProvisioning — harness for the Free tier's on-device model:
 ...
 *     node VerifyLocalLlmProvisioning.mjs
 *     VERIFY_BROWSER_LLM_HTTP=1 node VerifyLocalLlmProvisioning.mjs
 *
...
const LocalLlmModelRegistry = require("./Globals/Classes/LocalLlm/LocalLlmModelRegistry");
const LocalLlmModelCatalogue = require("./Globals/Constants/LocalLlmModelCatalogue");
const LocalLlmDownloadConstants = require("./Globals/Constants/LocalLlmDownloadConstants");
const { localLlmExecutionBackends } = require("./Globals/Enumerations/LocalLlmExecutionBackends");
const { localLlmModelKeys } = require("./Globals/Enumerations/LocalLlmModelKeys");
const { localLlmUnavailableReasons } = require("./Globals/Enumerations/LocalLlmUnavailableReasons");
...
{
    const selectorPath = path.join(repositoryRoot, "Main", "Globals", "Classes", "LocalLlm", "LocalLlmModelSelector.js");
    const profilePath = path.join(repositoryRoot, "Main", "Globals", "Classes", "LocalLlm", "LocalLlmDeviceProfile.js");
    ...
    return { LocalLlmModelSelector: selectorModule.default, LocalLlmDeviceProfile: profileModule.default };
}

function buildProfile(LocalLlmDeviceProfile, overrides)
{
    return new LocalLlmDeviceProfile(Object.assign(
    {
...
async function runSelectionTier()
{
    ...
    const { LocalLlmModelSelector, LocalLlmDeviceProfile } = await loadSelector();
    const allModelKeys = LocalLlmModelCatalogue.ORDER.slice();

    const selectFor = (overrides, availableKeys = allModelKeys, forcedKey = null) =>
        LocalLlmModelSelector.select(buildProfile(LocalLlmDeviceProfile, overrides), availableKeys, forcedKey);

    const bestGraphicsKey = allModelKeys.find((modelKey) =>
        LocalLlmModelCatalogue[modelKey].executionBackend === "WEBGPU");
    ...
    assert(
        LocalLlmModelCatalogue[withoutF16Outcome.getModelKey()].requiresShaderF16 === false,
    ...
    assert(
        phoneOutcome.getModelKey() !== bestGraphicsKey && LocalLlmModelCatalogue[phoneOutcome.getModelKey()].executionBackend === "WEBGPU",
    ...
    assert(
        !noGraphicsOutcome.isAvailable()
            && noGraphicsOutcome.getUnavailableReason() === localLlmUnavailableReasons.NO_SUPPORTED_BACKEND,
    ...
    assert(
        smallMemoryGraphicsOutcome.isAvailable()
            && LocalLlmModelCatalogue[smallMemoryGraphicsOutcome.getModelKey()].minimumDeviceMemoryGigabytes
    ...
    assert(
        !nothingOutcome.isAvailable()
            && nothingOutcome.getUnavailableReason() === localLlmUnavailableReasons.NO_SUPPORTED_BACKEND,
        "a browser with neither backend is refused, and told which capability is missing",
    );

    assert(
        selectFor(DESKTOP_GPU_WITH_F16, []).getUnavailableReason() === localLlmUnavailableReasons.NO_MODEL_PROVISIONED,
...
{
    const cataloguePath = path.join(repositoryRoot, "Main", "Globals", "Constants", "LocalLlmModelCatalogue.js");
    ...
    try
    {
        fs.writeFileSync(cataloguePath, `class LocalLlmModelCatalogue\n{\n${staticMembers}\n}\n\nmodule.exports = LocalLlmModelCatalogue;\n`, "utf8");
        fs.writeFileSync(scenarioFilePath, JSON.stringify(scenarios), "utf8");

        const childSource = `
            import fs from "fs";
            import { pathToFileURL } from "url";
            const selectorUrl = pathToFileURL(${JSON.stringify(path.join(repositoryRoot, "Main", "Globals", "Classes", "LocalLlm", "LocalLlmModelSelector.js"))}).href;
            const profileUrl = pathToFileURL(${JSON.stringify(path.join(repositoryRoot, "Main", "Globals", "Classes", "LocalLlm", "LocalLlmDeviceProfile.js"))}).href;
            const LocalLlmModelSelector = (await import(selectorUrl)).default;
            const LocalLlmDeviceProfile = (await import(profileUrl)).default;
            const scenarios = JSON.parse(fs.readFileSync(${JSON.stringify(scenarioFilePath)}, "utf8"));
            const results = scenarios.map((scenario) =>
            {
                const outcome = LocalLlmModelSelector.select(
                    new LocalLlmDeviceProfile(scenario.profile),
...
function runCatalogueIntegrityTier()
{
    ...
    const orderedKeys = LocalLlmModelCatalogue.ORDER;
    ...
    for (const modelKey of orderedKeys)
    {
        ...
        assert(localLlmModelKeys[modelKey] !== undefined, `${modelKey} has a matching enum key`);
        assert(
            localLlmExecutionBackends[descriptor.executionBackend] !== undefined,
...
function runWorkerProtocolTier()
{
    ...
    {
        LocalLlmWorkerCommands: JSON.parse(fs.readFileSync(path.join(repositoryRoot, "Common", "Enumerations", "LocalLlmWorkerCommands.json"), "utf8")),
        LocalLlmWorkerEvents: JSON.parse(fs.readFileSync(path.join(repositoryRoot, "Common", "Enumerations", "LocalLlmWorkerEvents.json"), "utf8")),
    };
...
function runSelfHostingTier()
{
    ...
    const constantsAreRootRelative = String(LocalLlmDownloadConstants.WORKER_SCRIPT_PATH).startsWith("/")
        && String(LocalLlmDownloadConstants.ENGINE_RUNNER_MODULE_PATH).startsWith("/")
        && String(LocalLlmDownloadConstants.ASSETS_BASE_PATH).startsWith("/");
    assert(
...
/**
 * The engine runner marks a lost GPU device by setting `error.name`, and the
 * session controller decides whether to retire the graphics backend by
 * comparing against its own copy of that string. The two cannot import each
 * other — one is bundled, the other deliberately is not — so the value is
 * duplicated by hand across the worker boundary.
 *
 * Worth asserting because the failure is silent and expensive. If the strings
 * drift, nothing throws and no test fails: a lost device simply stops being
 * recognised, the fallback to a re-selected model never fires, and the learner
 * is told the download failed on hardware that could have run the model. The
 * subsystem rename did exactly this — it rewrote one side and not the other.
 */
function runDeviceLostMirrorTier()
{
    section("The device-lost marker matches on both sides of the worker boundary");

    const runnerPath = path.join(repositoryRoot, "Main", "ThirdParty", "BrowserLlm", "BrowserLlmEngineRunner.js");
    const controllerPath = path.join(repositoryRoot, "Main", "Globals", "Classes", "LocalLlm", "LocalLlmSessionController.js");

    if (!fs.existsSync(runnerPath) || !fs.existsSync(controllerPath))
    {
        assert(false, "both sides of the device-lost mirror exist");
        return;
    }

    const readMarker = (filePath) =>
    {
        const markerMatch = fs.readFileSync(filePath, "utf8")
            .match(/DEVICE_LOST_ERROR_NAME\s*=\s*"([^"]+)"/);
        return markerMatch === null ? null : markerMatch[1];
    };

    const runnerMarker = readMarker(runnerPath);
    const controllerMarker = readMarker(controllerPath);

    assert(runnerMarker !== null, "the engine runner declares DEVICE_LOST_ERROR_NAME");
    assert(controllerMarker !== null, "the session controller declares DEVICE_LOST_ERROR_NAME");
    assert(
        runnerMarker !== null && runnerMarker === controllerMarker,
        `the two declarations are identical (runner "${runnerMarker}", controller "${controllerMarker}")`,
    );
}
...
function runManifestWalkTier()
{
    ...
    try
    {
        ...
        const collectedFiles = LocalLlmModelRegistry.collectFiles(path.join(modelsDirectory, "owner", "repo"));
        ...
        assert(
            LocalLlmModelRegistry.isModelComplete(descriptor, collectedFiles, path.join(modelsDirectory, "owner", "repo")) === true,
        ...
        assert(
            LocalLlmModelRegistry.isModelComplete(missingRequirement, collectedFiles, path.join(modelsDirectory, "owner", "repo")) === false,
        ...
        assert(
            LocalLlmModelRegistry.isModelComplete(
                emptyFileRequirement,
                LocalLlmModelRegistry.collectFiles(path.join(modelsDirectory, "owner", "repo")),
        ...
        assert(
            LocalLlmModelRegistry.isModelComplete(
                shardedDescriptor,
                LocalLlmModelRegistry.collectFiles(shardedDirectory),
                shardedDirectory
            ) === false,
        ...
        assert(
            LocalLlmModelRegistry.collectFiles(path.join(temporaryRoot, "does-not-exist")).length === 0,
    ...
    finally
    {
        ...
        LocalLlmModelRegistry.resetCache();
    }
}
...
function runEnabledModelsTier()
{
    ...
    const originalValue = process.env[LocalLlmModelRegistry.ENABLED_MODELS_ENVIRONMENT_VARIABLE];

    try
    {
        delete process.env[LocalLlmModelRegistry.ENABLED_MODELS_ENVIRONMENT_VARIABLE];
        assert(
            LocalLlmModelRegistry.getEnabledModelKeys().length === LocalLlmModelCatalogue.ORDER.length,
            "unset means every provisioned model is offered",
        );

        const firstKey = LocalLlmModelCatalogue.ORDER[0];
        process.env[LocalLlmModelRegistry.ENABLED_MODELS_ENVIRONMENT_VARIABLE] = firstKey;
        const restrictedKeys = LocalLlmModelRegistry.getEnabledModelKeys();
        assert(
            restrictedKeys.length === 1 && restrictedKeys[0] === firstKey,
            "a listed key narrows the set to exactly that model",
        );

        process.env[LocalLlmModelRegistry.ENABLED_MODELS_ENVIRONMENT_VARIABLE] = "NOT_A_REAL_MODEL";
        assert(
            LocalLlmModelRegistry.getEnabledModelKeys().length === 0,
    ...
    finally
    {
        ...
        {
            process.env[LocalLlmModelRegistry.ENABLED_MODELS_ENVIRONMENT_VARIABLE] = originalValue;
        }
        LocalLlmModelRegistry.resetCache();
    }
}
...
async function runHttpTier()
{
    ...
    try
    {
        manifestResponse = await fetch(`${baseUrl}${LocalLlmDownloadConstants.MANIFEST_ENDPOINT_PATH}`,
        {
    ...
    assert(
        typeof firstModel.baseUrl === "string" && firstModel.baseUrl.startsWith(LocalLlmDownloadConstants.ASSETS_BASE_PATH),
