...
class TermsAndConditionsManager
{
    ...
    // Resolves once the post-login legal flow has SETTLED — every required
    // document accepted, or there was nothing to accept (no documents seeded /
    // anonymous identity / fetch failure → fail-open). SyncOrchestrator awaits
    // this before starting any sync cycle: while acceptance is pending the
    // server 403s every sync endpoint (LEGAL_ACCEPTANCE_REQUIRED), and the
    // non-dismissible sync modal would otherwise mount on top of — and trap the
    // user behind — the legal-acceptance modal.
    static #legalSettledResolve = null;
    static #legalSettledPromise = new Promise(resolve => { TermsAndConditionsManager.#legalSettledResolve = resolve; });

    /**
     * Promise that resolves when the post-login legal flow has settled (all
     * required documents accepted, or nothing to accept). Resolves instantly
     * for already-accepted / returning users. Consumed by SyncOrchestrator.
     */
    static whenLegalSettled()
    {
        return TermsAndConditionsManager.#legalSettledPromise;
    }
    ...
    static async #handleUserLoggedIn(user)
    {
        ...
        {
            // Always release the sync gate once the flow settles — whether
            // every document was accepted or there was nothing to accept. The
            // decline path reloads the page, so a non-resolve there is moot.
            if (TermsAndConditionsManager.#legalSettledResolve)
            {
                TermsAndConditionsManager.#legalSettledResolve();
                TermsAndConditionsManager.#legalSettledResolve = null;
            }
        }
    }
