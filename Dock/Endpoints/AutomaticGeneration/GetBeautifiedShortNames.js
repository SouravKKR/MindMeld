 ...
 *
 * Flow: POST /Decks/BeautifyShortNames returns a taskId and runs the
 ...
 * the same 503 the old synchronous endpoint returned, so the UI keeps its
 * "model overloaded — retry" behaviour.
 *
 * Available to any signed-in user. The task id is an unguessable UUID; we
 * additionally refuse to serve a task the requester doesn't own while it is
 * still in Redis.
 */
async function getBeautifiedShortNames(request, response)
{
    ...
    // getTask returns null and we fall back to the unguessable-id guarantee.
    ...
    // publish deterministic names; the manual path instead surfaces the failure.
