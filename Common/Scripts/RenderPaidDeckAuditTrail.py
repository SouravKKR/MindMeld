...
def build_verification_sources(provenance):
    ...
    blocks = [Paragraph(
        "Documents and pages an administrator cleared and attached for use with this deck. Every "
        "attachment and every removal is recorded below with the licence declared for it at the time, "
        "the person who declared it, and what it was used for.",
        styles["body"],
    ), Paragraph(
        "The <b>Used for</b> column is the one to read first, because the three values support "
        "different claims. <b>Checking only</b> means the document was read solely by the verification "
        "pass, which runs after the content already exists and can only raise flags &mdash; nothing in "
        "the deck was written from it, so agreement between the two is independent corroboration. "
        "<b>Writing and checking</b> and <b>Writing only</b> both mean the deck's content was written "
        "from the document under the declared licence; the first was then checked back against it, the "
        "second deliberately was not. Section 1c names exactly which topics were written from which "
        "document.",
        styles["body"],
    )]
    ...
    #
    # The eighth column was funded by narrowing the ones whose contents wrap
    # gracefully — the source name, the licence and the URL all break at spaces
    # or punctuation. "Used for" is sized to hold "Writing and checking" on two
    # lines rather than three.
    blocks.append(make_table(
        ["Event", "Source", "Declared licence", "Used for", "URL", "Content hash", "Declared by", "When (UTC)"],
        rows,
        [1.35, 1.20, 1.20, 0.95, 1.00, 0.95, 1.05, 1.00],
    ))
...
USAGE_MODE_LABELS = {
    0: "Checking only",
    1: "Writing and checking",
    2: "Writing only",
}


def format_usage_mode(declaration):
    """
    What the declared document was actually used for.

    Defaults to "Checking only" for a declaration written before the field
    existed, which is what those records meant: at the time, checking was the
    only thing a declared source could be used for.

    An unrecognised value renders as such rather than being folded into the
    default. A report generated by an older renderer against a newer record must
    say it does not recognise the mode, not quietly assert the mildest one.
    """
    usage_mode = declaration.get("usageMode")

    if usage_mode is None:
        return html_escape(USAGE_MODE_LABELS[0])

    return html_escape(USAGE_MODE_LABELS.get(usage_mode, "Unrecognised"))
