 ...
 * Common/Constants/BrowserLlmModelCatalogue.json, which it reads directly
 ...
 * Usage:
 *
 *     node Common/Scripts/ProvisionBrowserLlmModels.js --list
 *     node Common/Scripts/ProvisionBrowserLlmModels.js
 *     node Common/Scripts/ProvisionBrowserLlmModels.js --models=QWEN2_5_1_5B_WEBGPU_Q4F16
 *     node Common/Scripts/ProvisionBrowserLlmModels.js --verify-only
 *
 * Options:
 ...
 */
class BrowserLlmModelProvisioner
{
    ...
    // The ONNX Runtime WebAssembly binary the vendored BrowserLlm.js bundle
    ...
    constructor(options)
    {
        ...
        this.catalogue = BrowserLlmModelProvisioner.readCatalogue(options.repositoryRoot);
    }

    static readCatalogue(repositoryRoot)
    {
        const cataloguePath = path.join(repositoryRoot, 'Common', 'Constants', 'BrowserLlmModelCatalogue.json');
    ...
    {
        return path.join(this.destinationRoot, ...BrowserLlmModelProvisioner.ONNX_RUNTIME_DIRECTORY_SEGMENTS);
    }
    ...
    async provisionModel(modelKey, descriptor)
    {
        ...
        console.log(`  ${plannedFiles.length} file(s), ${BrowserLlmModelProvisioner.formatBytes(plannedTotalBytes)}`);
    ...
    async planFiles(descriptor)
    {
        ...
        if (descriptor.executionBackend === 'WASM')
        {
            ...
            for (const repositoryEntry of repositoryEntries)
            {
                const bIsRootMetadataFile = !repositoryEntry.path.includes('/')
                    && !BrowserLlmModelProvisioner.IGNORED_REPOSITORY_FILE_NAMES.has(repositoryEntry.path);
                ...
                {
                    plannedFiles.push(BrowserLlmModelProvisioner.toPlannedFile(descriptor, repositoryEntry));
                }
            }
        ...
        for (const repositoryEntry of repositoryEntries)
        {
            if (BrowserLlmModelProvisioner.IGNORED_REPOSITORY_FILE_NAMES.has(repositoryEntry.path))
            {
                continue;
            }
            plannedFiles.push(BrowserLlmModelProvisioner.toPlannedFile(descriptor, repositoryEntry));
    ...
    static toPlannedFile(descriptor, repositoryEntry)
    {
        ...
        return {
            ...
            sourceUrl: `${BrowserLlmModelProvisioner.HUGGING_FACE_ORIGIN}/${descriptor.sourceRepository}/resolve/main/${repositoryEntry.path}`,
    ...
    {
        const treeUrl = `${BrowserLlmModelProvisioner.HUGGING_FACE_ORIGIN}/api/models/${sourceRepository}/tree/main?recursive=1`;
    ...
    isFileAlreadyValid(plannedFile, destinationPath)
    {
        ...
        {
            return BrowserLlmModelProvisioner.computeSha256(destinationPath) === plannedFile.expectedSha256;
        }
        return true;
    }
    ...
    async downloadFileWithRetries(plannedFile, destinationPath)
    {
        ...
        for (let attemptNumber = 1; attemptNumber <= BrowserLlmModelProvisioner.MAXIMUM_ATTEMPTS_PER_FILE; attemptNumber++)
        {
            try
            {
            ...
            catch (downloadError)
            {
                ...
                if (attemptNumber < BrowserLlmModelProvisioner.MAXIMUM_ATTEMPTS_PER_FILE)
                {
                    const delayMilliseconds = BrowserLlmModelProvisioner.RETRY_BASE_DELAY_MILLISECONDS * attemptNumber;
                    console.warn(`    retry ${attemptNumber}/${BrowserLlmModelProvisioner.MAXIMUM_ATTEMPTS_PER_FILE - 1}: ${plannedFile.relativePath} — ${downloadError.message}`);
    ...
    {
        const partialPath = destinationPath + BrowserLlmModelProvisioner.PARTIAL_FILE_SUFFIX;
        ...
        if (plannedFile.expectedSha256)
        {
            const writtenSha256 = BrowserLlmModelProvisioner.computeSha256(partialPath);
        ...
        console.log(`    fetched ${plannedFile.relativePath} (${BrowserLlmModelProvisioner.formatBytes(writtenSize)})`);
    }
        ...
        try
        {
            ...
            if (largestBufferBytes > 0)
            {
                ...
                const suffix = largestBufferBytes > declaredMinimum
                    ...
                    : '';
                console.log(`  largest parameter buffer: ${BrowserLlmModelProvisioner.formatBytes(largestBufferBytes)} (catalogue minimum ${BrowserLlmModelProvisioner.formatBytes(declaredMinimum)})${suffix}`);
            }
        }
        catch (manifestError)
        {
    ...
    writeProvisionManifest(modelKey, descriptor, modelDirectory, outcomes)
    {
        ...
        fs.writeFileSync(
            path.join(modelDirectory, BrowserLlmModelProvisioner.PROVISION_MANIFEST_FILE_NAME),
    ...
    async provisionOnnxRuntime()
    {
        ...
        for (const runtimeFileName of BrowserLlmModelProvisioner.ONNX_RUNTIME_FILE_NAMES)
        {
            ...
            await this.downloadFileWithRetries(
            {
                ...
                sourceUrl: `${BrowserLlmModelProvisioner.ONNX_RUNTIME_SOURCE_URL}${runtimeFileName}`,
                sizeBytes: 0,
                expectedSha256: null,
            }, destinationPath);
        }
    ...
    static parseCommandLineArguments(argumentList, repositoryRoot)
    {
        const parsed =
        {
            ...
            bVerifyOnly: false,
            bList: false,
            concurrency: BrowserLlmModelProvisioner.DEFAULT_CONCURRENCY,
        };
        ...
        for (const argument of argumentList)
        {
            ...
            else if (argument.startsWith('--concurrency='))
            {
                ...
                parsed.concurrency = Number.isFinite(parsedConcurrency) && parsedConcurrency > 0
                    ? parsedConcurrency
                    : BrowserLlmModelProvisioner.DEFAULT_CONCURRENCY;
            }
            else
            {
...
(async () =>
{
    ...
    try
    {
        const options = BrowserLlmModelProvisioner.parseCommandLineArguments(process.argv.slice(2), repositoryRoot);
        const provisioner = new BrowserLlmModelProvisioner(options);
...
module.exports = BrowserLlmModelProvisioner;
