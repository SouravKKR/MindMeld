...
expensive halves of the CogniumLearn image pipeline:
  ...
  1. Layout figure detection         (Agent/Workflows/PrepareImages/ImageExtractor.py)
...
Output, per run, under  Common/Testing/Experimental/Output/<pdf>_<timestamp>/:
    ...
    01_layout_detected/  every crop the layout model (+ vector/embedded fallbacks) found
...
Requires the Agent's virtualenv (it has transformers, google-genai,
...
#     repo's own source of truth for billing normalisation. They cover the layout
...
# The repo's current picture-confidence floor, shown as the default in the UI.
# The real value lives on DoclingLayoutDetector.PICTURE_CONFIDENCE_THRESHOLD; the
# harness passes an override into the detector's constructor rather than mutating
# a class attribute, so the production code path is exercised, not modified.
DEFAULT_PICTURE_CONFIDENCE_THRESHOLD = 0.20
...
async def run_pipeline(
    ...
    picture_confidence_threshold: float,
    run_directory: str,
    log,
    ...
    layout_detected_directory = os.path.join(run_directory, "01_layout_detected")
    ...
    for directory in (layout_detected_directory, llm_confirmed_directory, enhanced_directory):
        os.makedirs(directory, exist_ok=True)

    report: dict = {
        "pdf": pdf_path,
        "pageFilter": sorted(allowed_pages) if allowed_pages else "ALL PAGES",
        "pictureConfidenceThreshold": picture_confidence_threshold,
    ...
    try:
        ...
        # ----------------------------------------------------------------
        log("=" * 70)
        log("STAGE 1 / LAYOUT DETECTION")
        log("=" * 70)
        log(f"Picture confidence floor -> {picture_confidence_threshold} "
            f"(repo default {DEFAULT_PICTURE_CONFIDENCE_THRESHOLD}).")

        # Injected into the real detector -- the optimisation knob. Nothing is
        # monkey-patched, so this exercises the production path rather than a
        # mutated copy of it.
        layout_detector = DoclingLayoutDetector(
            picture_confidence_threshold=picture_confidence_threshold
        )

        with open(pdf_path, "rb") as pdf_file:
            pdf_bytes = pdf_file.read()
        log(f"Loaded PDF ({len(pdf_bytes):,} bytes).")
        log("Running layout detection... first run downloads the Docling Heron "
            "weights from Hugging Face and can take a minute. Please wait.")

        import asyncio
        detection_started = time.perf_counter()
        extractor = ImageExtractor(layout_detector=layout_detector)
        ...
        log(f"Layout detection finished in {detection_seconds:.1f}s -- "
        ...
        for figure_index, figure in enumerate(extracted_figures):
            ...
            with open(os.path.join(layout_detected_directory, file_name), "wb") as image_file:
        ...
        with open(os.path.join(layout_detected_directory, "layout_detections.json"), "w", encoding="utf-8") as manifest_file:
        ...
        log(f"Picture confidence:  {picture_confidence_threshold}")
        log(f"Detected (layout):   {len(extracted_figures)}")
...
def build_text_report(report: dict) -> str:
    lines = []
    lines.append("CogniumLearn Layout + Image-Enhance Lab -- run report")
    lines.append("=" * 60)
    lines.append(f"PDF:               {report.get('pdf')}")
    lines.append(f"Pages:             {report.get('pageFilter')}")
    lines.append(f"Picture confidence: {report.get('pictureConfidenceThreshold')}")
    lines.append(f"Started:           {report.get('startedAt')}")
    lines.append(f"Finished:          {report.get('finishedAt')}")
    lines.append("")

    layout = report.get("layout", {})
    lines.append(f"[1] Layout detection: {layout.get('detectedCount', 0)} figure(s) "
                 f"in {layout.get('seconds', 0)}s")
    for page_number, count in sorted((layout.get("perPage") or {}).items()):
...
def launch_gui():
    ...
    root.title("CogniumLearn -- Layout + Image Enhance Lab")
    root.geometry("960x680")

    selected_pdf_path_variable = tkinter_module.StringVar(value="")
    page_range_variable = tkinter_module.StringVar(value="")
    picture_confidence_variable = tkinter_module.StringVar(value=str(DEFAULT_PICTURE_CONFIDENCE_THRESHOLD))
    ...
    # Row 2 -- picture confidence
    tkinter_module.Label(controls_frame, text="Picture confidence floor:").grid(row=2, column=0, sticky="w", pady=(8, 0))
    tkinter_module.Entry(controls_frame, textvariable=picture_confidence_variable, width=30).grid(row=2, column=1, sticky="w", padx=5, pady=(8, 0))
    tkinter_module.Label(controls_frame, text=f"repo default {DEFAULT_PICTURE_CONFIDENCE_THRESHOLD} -- raise to detect fewer / cleaner figures").grid(row=2, column=1, sticky="e", pady=(8, 0))
    ...
    def worker_main(pdf_path, allowed_pages, enhance_enabled, maximum_images, picture_confidence, run_directory, stop_event):
        import asyncio
        ...
        try:
            asyncio.run(run_pipeline(
                ...
                picture_confidence_threshold=picture_confidence,
    ...
    def start_run():
        ...
        try:
            picture_confidence = float(picture_confidence_variable.get().strip())
            if not (0.0 < picture_confidence < 1.0):
                raise ValueError()
        except ValueError:
            messagebox.showerror("Bad confidence", "Picture confidence must be a number between 0 and 1.")
            return
        ...
        worker_thread = threading.Thread(
            ...
            args=(pdf_path, allowed_pages, enhance_enabled_variable.get(),
                  maximum_images, picture_confidence, run_directory, stop_event),
            daemon=True,
        )
        worker_thread.start()
